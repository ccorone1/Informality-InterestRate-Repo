%% Data processing and SVAR with Chebyshev (multiple p)
%
% Versión: multi-p (Elizondo)
...
m        = 30;                 % orden máximo Chebyshev
m_star   = 3;                  % grado fijo del polinomio a usar
p_grid   = [1 2 3 4 6];       % lags del VAR a comparar
s        = 32;                  % horizonte IRF
...
if monthly_data
    ...
    try
        dt0 = datetime(raw_periods,"InputFormat","uuuu'Q'Q");
    catch
        dt0 = datetime(raw_periods);
    end
    dt0     = dateshift(dt0,'end','quarter');   % fin de trimestre
    periods = string(dt0);
end
...
if monthly_data
    ...
    try
        dt = datetime(periods_cma);  % '31-Mar-2005' etc.
    catch
...
% Polinomio Chebyshev fijo (m_star)
assert(m_star >= 0 && m_star <= m, 'm_star fuera de rango.');
...
%% 5. VARX (Uhlig) para múltiples p

data = endo;
[~, k] = size(data);
ne     = k;

% Elegir choque (mismo para todos los p)
...
IRF_all = cell(length(p_grid), 1);

for idx_p = 1:length(p_grid)
    p_current = p_grid(idx_p);
    fprintf('\n--- Estimando VARX para p = %d ---\n', p_current);

    % Matriz de exógenas completa: [Chebyshev, otras]
    exo = [double(exo_single), double(exo_external_final)];

    % Estimación VARX
    [betas, ~, omega, res] = estimavarx(data, p_current, exo);

    n_exo_total = size(exo,2);
    betas_imp   = betas(:,1:end-n_exo_total);  % quitamos exógenas

    % Para la covarianza de B (depende de p)
    xx = momentxx(data, p_current);
    ...
    for rep = 1:reps
        ...
        resp = impulse(storeb, v_imp, s, p_current);   % s x ne

        IRF_draws(:,:,rep) = resp;
    end

    % Mediana en la dimensión de draws
    IRF_all{idx_p} = median(IRF_draws, 3);
end

%% 6. Gráfica: IRFs para distintos p
...
colors = lines(length(p_grid));

figure('Name','IRFs VARX con Chebyshev (multi p)','Color','w');
if use_precovid
...
sgtitle(sprintf('IRFs (shock: %s) | m*=%d, s=%d, %s, %s, Exo=%s', ...
    variable_names{IMP}, m_star, s, cutoff_text, time_granularity, txt_exo), ...
    'FontSize', 13);
...
for ii = 1:ne
    ...
    for idx_p = 1:length(p_grid)
        IRm = IRF_all{idx_p}(:, ii);      % s x 1
        plot(x, IRm, 'LineWidth', 1.4, 'Color', colors(idx_p,:));
    end
    ...
    if ii == 1
        legend("p="+string(p_grid),'Location','best');
    end
end
...
save('varx_irf_results_multi_p.mat', ...
     'IRF_all','results_meta');

toc;
fprintf('Saved as "varx_irf_results_multi_p.mat".\n');
